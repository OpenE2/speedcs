        ...
        Try
            ...
            Select Case sRequestedFile
                ...
                Case "requests.html"
                    ...
                    sMessage.Append("</tr>")
                    For Each req As clsCache.clsCAMDMsg In Cache.Requests
                        sMessage.Append("<tr>")
                        Dim c As clsSettingsClients.clsClient = CfgClients.Clients.FindByUCRC(req.usercrc)
                        If Not c Is Nothing Then
                            sMessage.Append("<td>" & DateDiff(DateInterval.Second, req.reqtime, Now) & "Sec.</td><td>" & req.ecmcrc.ToString("X8") & "</td><td>" & c.Username & "[" & req.usercrc.ToString("X4") & "]</td><td>" & req.ServiceName & "</td><td>" & req.ClientPID.ToString("X4") & "</td><td>" & req.unknown.ToString("X4") & "</td>")
                        Else
                            sMessage.Append("<td>" & DateDiff(DateInterval.Second, req.reqtime, Now) & "Sec.</td><td>" & req.ecmcrc.ToString("X8") & "</td><td>" & req.usercrc.ToString("X4") & "</td><td>" & req.ServiceName & "</td><td>" & req.ClientPID.ToString("X4") & "</td><td>" & req.unknown.ToString("X4") & "</td>")
                        End If
                        sMessage.Append("</tr>")
                    Next
                    ...
                    sMessage.Append("<table border=1 width='100%'>")
                    sMessage.Append("<tr>")
                    sMessage.Append("<th>Age</th><th>ECM CRC</th><th>Servicename</th><th>ClientPID</th>") '<th>Unique</th><th>CMD</th><th>Len</th><th colspan=2>?</th><th colspan=4>CRC ECM</th><th colspan=2>SrvId</th><th colspan=2>Card</th><th colspan=4>PrId</th><th colspan=2>Idx</th><th colspan=2>?</th><th colspan=16>ECM</th>
                    sMessage.Append("</tr>")

                    For Each req As clsCache.clsCAMDMsg In Cache.Answers
                        sMessage.Append("<tr>")
                        sMessage.Append("<td>" & DateDiff(DateInterval.Second, req.reqtime, Now) & "Sec.</td><td>" & req.ecmcrc.ToString("X8") & "</td><td>" & req.ServiceName & "</td><td>" & req.ClientPID.ToString("X4") & "</td>")
                    Next
                    sMessage.Append("</table>")
